...
def train_step(model, train_loader, optimizer, scheduler, weight=None):
    total_loss = 0
...
def train(args):
    (tag_values, tag2idx), (input_ids, tags, attention_mask), weights = create_dataset(args, False)
    ...
    for fold_idx, (train_index, test_index) in enumerate(kf.split(input_ids)):  # for every fold
        ...
        for epoch in range(args.epoch):
            ...
            # train
            avg_train_loss = train_step(model, train_loader, optimizer, scheduler, weights)
